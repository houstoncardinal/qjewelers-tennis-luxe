// Full country list for the checkout/address country selector. `name` is what
// we store on orders/addresses (unchanged, human-readable, backward-compatible
// with existing rows); `iso2` is only used to talk to the Stripe Tax API.
...
const NAME_TO_ISO = new Map(COUNTRIES.map((c) => [c.name, c.iso2]));

// Stripe Tax requires ISO 3166-1 alpha-2. We store the full display name on
// orders (unchanged historical format), so this converts only at the API edge.
export function countryNameToISO(name: string): string | null {
  return NAME_TO_ISO.get(name.trim()) ?? null;
}
