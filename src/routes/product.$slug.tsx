...
import {
  ...
  LENGTH_BRACELET_DEFAULT,
  TENNIS_BRACELET_LENGTH_DEFAULT,
  TENNIS_CHAIN_LENGTH_DEFAULT,
  type Size,
  type EarringSize,
  type RingSize,
  type Length,
  ...
  COLOR_SHORT,
  SIZE_DESCRIPTIONS,
  EARRING_SIZE_DESCRIPTIONS,
  RING_SIZE_DESCRIPTIONS,
  TENNIS_BRACELET_SIZE_DESCRIPTIONS,
  TENNIS_BRACELET_LENGTH_DESCRIPTIONS,
  TENNIS_CHAIN_SIZE_DESCRIPTIONS,
  TENNIS_CHAIN_LENGTH_DESCRIPTIONS,
  LENGTH_DESCRIPTIONS,
  BRACELET_LENGTH_DESCRIPTIONS,
  MOISSANITE_QUALITY,
} from "@/lib/pricing";
import { useCart } from "@/lib/cart";
import { EmailCapture } from "@/components/marketing/email-capture";
import { showCartAddedToast } from "@/components/cart-added-toast";
...
function ShareBar({
  ...
  const SHARES = [
    ...
    },
    {
      ...
      color:
        "hover:bg-gradient-to-tr hover:from-[#f09433] hover:via-[#e6683c] hover:to-[#dc2743] hover:text-white hover:border-[#e6683c]",
      copyInstead: true,
    },
  ];
  ...
  return (
    <div className="pt-4 border-t border-gray-100">
      <p className="text-[0.58rem] uppercase tracking-[0.20em] text-gray-400 mb-3">
        Share this piece
      </p>
...
export const Route = createFileRoute("/product/$slug")({
  loader: async ({ params }) => {
    ...
    const sibSlug = isRingL
      ? null
      : isBraceletL
        ? p.slug.replace("bracelet", "chain")
        : isEarringL
          ? p.slug.replace("stud-earrings", "tennis-chain")
  ...
  head: ({ loaderData, params }) => {
    ...
    const imageUrl = p.image_url?.startsWith("http")
      ? p.image_url
      : `${SITE}${p.image_url || "/QURESHIJEWELERSLOGO.png"}`;

    const galleryUrls = (loaderData?.galleryImages ?? []).map((g: any) =>
      g.url?.startsWith("http") ? g.url : `${SITE}${g.url}`,
    );
          ...
          children: JSON.stringify({
            ...
            about: [
              {
                "@type": "Thing",
                name: "Moissanite",
                sameAs: "https://en.wikipedia.org/wiki/Moissanite",
              },
              {
                "@type": "Thing",
            ...
            offers: {
              ...
              hasMerchantReturnPolicy: {
                ...
                description:
                  "All sales final. Size/length exchanges accepted within 7 days on unworn items; customer pays return shipping. Damaged-on-arrival items replaced within 48 hours of delivery report.",
              },
              warranty: {
                ...
                warrantyScope:
                  "Manufacturing defects including stone loss, clasp failure, and plating defects under normal wear. $50–$100 deductible applies.",
              },
            },
  ...
  errorComponent: ({ error }) => (
    <div className="mx-auto max-w-2xl px-6 py-24 text-center">
      ...
      <Link
        to="/shop"
        className="inline-block mt-6 text-xs uppercase tracking-[0.2em] border-b border-foreground"
      >
        Back to shop
      </Link>
    </div>
...
function AccordionSection({
  ...
  return (
    <div className="border-b border-border">
      ...
      {open && <div className="pb-7 animate-fade-in">{children}</div>}
    </div>
  );
}
...
function ReviewForm({ slug, onSuccess }: { slug: string; onSuccess: () => void }) {
  ...
  const inputCls =
    "w-full border border-[#ddd8d0] bg-white px-3 py-2.5 text-sm focus:outline-none focus:border-foreground transition-colors";
    ...
    <form onSubmit={handleSubmit} className="mt-6 space-y-3 border-t border-border pt-6">
      ...
      <div className="grid sm:grid-cols-2 gap-3">
        ...
        <input
          ...
          placeholder="Order # (optional — for verified badge)"
          className={inputCls}
        />
      </div>
      <input
      ...
      <button
        type="submit"
        disabled={loading}
        className="bg-foreground text-background px-6 py-2.5 text-[0.58rem] uppercase tracking-[0.22em] disabled:opacity-50 hover:bg-foreground/90 transition-colors flex items-center gap-2"
      >
        {loading && (
...
function ProductPage() {
  ...
  const ankletLengths: string[] =
    ankletVariants.length > 0
  ...
  const pendantColors: string[] =
    pendantVariants.length > 0
      ? [
          ...new Set(
            pendantVariants.map((v) => v.color).filter((c): c is string => !!c && !!COLOR_MAP[c]),
          ),
        ]
      : (product.color ?? "gold")
          .split(",")
  ...
  const earringColors: string[] =
    earringVariants.length > 0
      ? [
          ...new Set(
            earringVariants.map((v) => v.color).filter((c): c is string => !!c && !!COLOR_MAP[c]),
          ),
        ]
      : (product.color ?? "gold")
          .split(",")
          .map((c: string) => c.trim())
          .filter((c: string) => !!COLOR_MAP[c]);
  const earringSizes: string[] =
    earringVariants.length > 0
      ? [...new Set(earringVariants.map((v) => v.size).filter((s): s is string => !!s))].sort(
          (a, b) => parseFloat(a) - parseFloat(b),
        )
      : [...SIZES_EARRING];
  ...
  const closureSizes = hasClosureOptions
    ? earringSizes.filter((s) =>
        (closureType === "screw_back" ? SCREW_BACK_SIZES : FRICTION_BACK_SIZES).has(s),
      )
    : earringSizes;
  ...
  const tennisSizeImgIdx: Record<string, number> = {
    "2mm": 4,
    "3mm": 5,
    "4mm": 6,
    "5mm": 7,
  ...
  const earringVideo =
    isEarring && (slug.includes("3-prong") || slug.includes("3prong"))
      ? productImages.earringVideo
      : null;
  ...
  const showImage = (i: number) => {
    setColorOverrideUrl(null);
    setActiveImg(i);
  };
  const showColorImage = (color: string, fallbackIdx: number) => {
    const ci = colorImages[color];
    if (ci) {
      setColorOverrideUrl(ci);
    } else {
      setColorOverrideUrl(null);
      setActiveImg(Math.min(fallbackIdx, gallery.length - 1));
    }
  };
  ...
  const handleAdd = (goToCart = false) => {
    ...
    const cartClosure = hasClosureOptions
      ? closureType === "screw_back"
        ...
        : "Friction Back"
      : undefined;
    const cartImage = isTennis
      ? tennisMetal === "white_gold"
        ? gallery[1]
        : gallery[0]
      : isEarring
        ? earringMetal === "white_gold"
          ? gallery[1]
          : gallery[0]
        : gallery[0];
    add(
    ...
    if (!goToCart) {
      showCartAddedToast({
        ...
        details: [size, cartLength, COLOR_MAP[cartColor]?.label ?? cartColor, cartClosure]
          .filter(Boolean)
          .join(" · "),
        quantity: qty,
        unitPrice: price,
        onViewBag: () => navigate({ to: "/cart" }),
      });
    }
    ...
    : isEarring
      ...
      : isRing
        ? product.slug.replace(
            /.*-solitaire-moissanite-ring/,
            `${product.color}-moissanite-tennis-chain`,
          )
        : product.slug.replace("chain", "bracelet");

  const breadcrumbType = isRing
    ? "ring"
    : isBracelet
      ? "bracelet"
      : isEarring
        ...
        ? [
            ...
            hasClosureOptions
              ? closureType === "screw_back"
                ...
                : "Friction Back"
              : null,
            earringMetal === "white_gold" ? "White Gold" : "Yellow Gold",
          ]
            .filter(Boolean)
            .join(" · ")
        : isRing
      ...
      <div className="bg-background overflow-x-hidden">
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-10 pt-5 sm:pt-7 lg:pt-9">
          <nav className="flex flex-wrap items-center gap-x-2.5 gap-y-1 text-[0.55rem] uppercase tracking-[0.22em] text-muted-foreground">
            ...
            <Link
              to="/shop"
              search={{ type: breadcrumbType as any }}
                  ...
                  <div className="absolute top-3.5 left-3.5 bg-white/92 backdrop-blur-sm border border-border/40 px-3 py-2 flex items-center gap-2 shadow-sm">
                    ...
                    <span className="text-[0.46rem] uppercase tracking-[0.24em] font-medium">
                      {isTennis
                        ...
                        : (earringMetal === "white_gold" ? "18K White Gold" : "18K Yellow Gold") +
                          " Selected"}
                    </span>
                  </div>
                ) : colorInfo ? (
            ...
            <div className="min-w-0 lg:sticky lg:top-6 lg:self-start">
              ...
              <div className="flex items-center gap-2.5 mb-4 sm:mb-5">
                <div
                  ...
                  style={{
                    background: "linear-gradient(to right, oklch(0.72 0.10 80), transparent)",
                  }}
                />
              ...
              <div className="mb-6">
                <div className="flex items-baseline gap-3 mb-1.5">
                  ...
                  {isEarring && (
                    <span className="text-[0.48rem] text-muted-foreground/60 uppercase tracking-[0.10em] ml-1">
                      per pair
                    </span>
                  )}
                </div>
                      ...
                      <div className="flex items-center gap-2 text-[0.54rem] text-muted-foreground">
                        ...
                        <span>
                          ...
                          <span className="line-through opacity-50">
                            {diamondMap[size] ?? "$2,000"}+
                          </span>{" "}
                          —{" "}
                          <span className="text-foreground font-medium">
                            same brilliance, fraction of the cost
                          </span>
                        </span>
                      </div>
                    );
              ...
              {isTennis && (
                <div className="mb-6">
                    ...
                    return (
                      <div className="flex items-baseline justify-between mb-3.5">
                        ...
                        <span className="text-[0.57rem] italic text-muted-foreground">
                          {metalDescriptions[tennisMetal] ??
                            COLOR_MAP[tennisMetal]?.label ??
                            "Fine metal"}
                        </span>
                      </div>
                    );
                  ...
                  <div
                    className={`grid gap-2.5 ${tennisColors.length >= 3 ? "grid-cols-3" : "grid-cols-2"}`}
                  >
              ...
              {isRing && (
                <div className="mb-6">
                  ...
                  <div
                    className={`grid gap-2.5 ${ringColors.length === 1 ? "grid-cols-1" : ringColors.length === 2 ? "grid-cols-2" : "grid-cols-3"}`}
                  >
                    {ringColors.map((key) => {
                      const info = COLOR_MAP[key];
              ...
              {isPendant && pendantColors.length > 0 && (
                <div className="mb-6">
                  ...
                  <div
                    className={`grid gap-2.5 ${pendantColors.length === 1 ? "grid-cols-1" : pendantColors.length === 2 ? "grid-cols-2" : "grid-cols-3"}`}
                  >
              ...
              {isEarring && (
                <div className="mb-6">
                  <div className="flex items-baseline justify-between mb-3.5">
                    ...
                    <span className="text-[0.57rem] italic text-muted-foreground">
                      {earringMetal === "white_gold"
                        ? "Cool · Icy white brilliance"
                        : earringMetal === "rose_gold"
                          ? "Warm · Romantic rose tone"
                          : "Warm · Classic yellow lustre"}
                    </span>
                  </div>
                  {earringColors.length === 1 ? (
                    ...
                    <div className="relative py-5 text-center border border-foreground bg-foreground text-background flex flex-col items-center justify-center gap-2">
                      ...
                      />
                      <span className="text-[0.70rem] font-semibold leading-none">
                        {COLOR_MAP[earringColors[0]]?.label ?? earringColors[0]}
                      </span>
                      <span className="text-[0.40rem] uppercase tracking-[0.16em] text-background/50">
                        5× plated
                      </span>
                      <span
                        className="absolute bottom-0 left-0 right-0 h-[2px]"
                    ...
                    <div
                      className={`grid gap-2.5 ${earringColors.length === 2 ? "grid-cols-2" : earringColors.length === 3 ? "grid-cols-3" : "grid-cols-4"}`}
                    >
              ...
              {hasClosureOptions && (
                <div className="mb-6">
                  <div className="flex items-baseline justify-between mb-3.5">
                    ...
                    <span className="text-[0.57rem] italic text-muted-foreground">
                      {closureType === "screw_back"
                        ? "Threads secure · best for daily retention"
                        : "Quick on/off · push-back butterfly"}
                    </span>
                  </div>
                    ...
                    {[
                      ...
                      return (
                        ...
                        >
                          ...
                          <span
                            className={`text-[0.40rem] uppercase tracking-[0.12em] ${active ? "text-background/70" : "text-muted-foreground/60"}`}
                          >
                            {note}
                          </span>
                          <span
                            className={`text-[0.36rem] uppercase tracking-[0.08em] ${active ? "text-background/50" : "text-muted-foreground/40"}`}
                          >
                            {sub}
                          </span>
                          {active && (
                ...
                >
                  {(sizes as readonly string[]).map((s) => {
                    ...
                    return (
                      ...
                      >
                        <span className="block text-[0.74rem] font-semibold leading-none mb-1">
                          {s.replace(/^Ring Size\s*/i, "")}
                        </span>
                        <span
                          className={`block text-[0.44rem] uppercase tracking-[0.08em] ${active ? "text-background/50" : "text-muted-foreground/55"}`}
                        >
                          {formatUSD(sp)}
                        </span>
                        {active && (
                ...
                (() => {
                  ...
                  return (
                    <div className="mt-3 mb-6 px-4 py-5 bg-cream border border-border">
                      <p className="text-[0.42rem] uppercase tracking-[0.22em] text-muted-foreground mb-5">
                        Size on ear · to scale
                      </p>
                      <div className="flex items-end justify-around gap-1">
                        {earringSizes.map((s) => {
                          ...
                          return (
                            ...
                            >
                              <span
                                className={`text-[0.34rem] uppercase tracking-[0.08em] leading-none ${active ? "text-muted-foreground" : "text-muted-foreground/50"}`}
                              >
                                {CARAT_LABEL[s] ?? ""}
                              </span>
                              <div
                              ...
                              />
                              <span
                                className={`text-[0.40rem] font-semibold uppercase tracking-[0.10em] leading-none ${active ? "text-foreground" : "text-muted-foreground"}`}
                              >
                                {s}
                              </span>
                              <span
                                className={`text-[0.32rem] uppercase tracking-[0.08em] ${active ? "text-muted-foreground" : "text-muted-foreground/40"}`}
                              >
                                {SCALE_LABEL[s] ?? ""}
                              </span>
                            </button>
              ...
              {isRing && (
                <div className="mb-6 flex items-start gap-3 bg-cream border border-border px-4 py-4">
                  ...
                  <p className="text-[0.60rem] text-muted-foreground leading-[1.75]">
                    ...
                    <strong className="text-foreground font-medium">order notes</strong> at
                    checkout.{" "}
                    <Link
                      to="/size-guide"
                ...
                <div className="mb-6 border border-border overflow-hidden">
                  ...
                  <div className="px-4 py-4 bg-cream">
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        <p className="text-[0.44rem] uppercase tracking-[0.22em] text-muted-foreground mb-1">
                          Anklet Length
                        </p>
                        <p className="text-[0.86rem] font-semibold leading-none">
                          {ankletSingleLength}
                        </p>
                        <p className="text-[0.48rem] text-muted-foreground/70 mt-1.5">
                          +2 inches adjustable · fits up to{" "}
                          {(() => {
                            const base = parseFloat(ankletSingleLength);
                            return `${base + 2}"`;
                          })()}
                        </p>
                      </div>
                      <div className="text-right shrink-0">
                        <p className="text-[0.44rem] uppercase tracking-[0.18em] text-muted-foreground mb-1">
                          Width
                        </p>
              ...
              {isPendant && pendantSingleLength && (
                <div className="mb-6 flex items-center justify-between px-4 py-3.5 bg-cream border border-border">
                  <div>
                    <p className="text-[0.44rem] uppercase tracking-[0.22em] text-muted-foreground mb-0.5">
                      Chain Length
                    </p>
                    <p className="text-[0.76rem] font-semibold leading-none">
                      {pendantSingleLength}
                    </p>
                  </div>
              ...
              {isPendant && !pendantSingleLength && (
                <div className="mb-6">
                  ...
                  <div
                    className={`grid gap-1.5 ${pendantLengths.length <= 3 ? `grid-cols-${pendantLengths.length}` : "grid-cols-3"}`}
                  >
                    {pendantLengths.map((l) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <span
                            className={`block text-[0.42rem] uppercase tracking-[0.08em] ${active ? "text-background/50" : "text-muted-foreground/55"}`}
                          >
                ...
                <div className="mb-6">
                  <div className="flex items-baseline justify-between mb-3.5">
                    ...
                    {!isTennis && (
                      <Link
                        to="/size-guide"
                        className="text-[0.52rem] text-muted-foreground hover:text-foreground transition-colors"
                      >
                        Size guide →
                      </Link>
                    )}
                    ...
                    {(isTennisChain
                      ...
                      const desc = isTennisChain
                        ...
                        : isTennis
                          ...
                          : isBracelet
                            ? BRACELET_LENGTH_DESCRIPTIONS[l]
                            : LENGTH_DESCRIPTIONS[l];
                      const active = length === l;
                      return (
                        <button
                        ...
                        >
                          <span className="block text-[0.72rem] sm:text-[0.76rem] font-semibold leading-none mb-1">
                            {l}
                          </span>
                          <span
                            className={`block text-[0.42rem] sm:text-[0.42rem] uppercase tracking-[0.08em] ${active ? "text-background/50" : "text-muted-foreground/55"}`}
                          >
                            {formatUSD(lp)}
                          </span>
                          {active && (
                ...
                <div className="mb-6 border border-border overflow-hidden">
                  ...
                  <div className="px-4 py-4 flex items-center justify-between gap-3 bg-cream">
                    <div className="min-w-0 flex-1">
                      <p className="text-[0.40rem] uppercase tracking-[0.24em] text-muted-foreground mb-1.5">
                        Your selection
                      </p>
                      {isTennis ? (
                        <>
                        ...
                        <>
                          ...
                          <p className="text-[0.48rem] text-muted-foreground/65 mt-1.5">
                            VVS1 · {product.slug.includes("black") ? "4-Prong" : "3-Prong"} · GRA
                            Certified
                          </p>
                        </>
                      )}
                    ...
                    <div className="text-right shrink-0">
                      <p className="font-display text-[1.45rem] sm:text-[1.65rem] leading-none">
                        {formatUSD(price)}
                      </p>
                      {isEarring && (
                        <p className="text-[0.38rem] text-muted-foreground/50 uppercase tracking-[0.12em] mt-1">
                          per pair
                        </p>
                      )}
                    </div>
              ...
              <div className="flex gap-2 mb-2.5">
                <div className="flex items-stretch border border-border shrink-0">
                  ...
                  <span className="w-9 flex items-center justify-center text-sm font-medium">
                    {qty}
                  </span>
                  <button
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-px bg-border/60 border border-border/60 mb-6">
                {[
                  ...
                  >
                    ...
                    <span className="text-[0.38rem] text-muted-foreground/55 uppercase tracking-[0.08em]">
                      {sub}
                    </span>
                  </div>
                ))}
              ...
              <ShareBar
                ...
                imageUrl={
                  product.image_url?.startsWith("http")
                    ? product.image_url
                    : `${SITE_URL}${product.image_url || "/QURESHIJEWELERSLOGO.png"}`
                }
              />
            </div>
          </div>
          ...
          <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-10 pb-16">
              ...
              <dl className="border border-border mb-7 overflow-hidden">
                ...
                {(isAnklet
                  ? [
                      ...
                      {
                        k: "Stone",
                        v: "VVS1 Moissanite · D Colorless · Round Brilliant",
                        hl: false,
                      },
                  ...
                  : isTennisChain
                    ? [
                        {
                          k: "Material",
                          v: "Solid S925 Sterling Silver (92.5% purity)",
                          hl: false,
                        },
                    ...
                    : isTennis
                      ...
                      : isEarring
                        ? ([
                            ...
                            {
                              k: "Stone",
                              v: product.slug.includes("black")
                                ? "VVS1 Black Moissanite · Round Brilliant Cut"
                                : "VVS1 Moissanite · D Colorless · Round Brilliant Cut",
                              hl: false,
                            },
                            {
                              k: "Setting",
                              v:
                                product.slug.includes("3-prong") || product.slug.includes("3prong")
                                  ? "3-Prong Round Brilliant"
                                  : "Classic 4-Prong Basket",
                              hl: false,
                            },
                            ...(product.slug.includes("black")
                              ? [
                                  {
                                    k: "Sizes",
                                    v: "5mm (0.5ct) · 6mm (0.8ct) · 6.5mm (1ct) · 8mm (2ct) per stone",
                                    hl: false,
                                  },
                                  {
                                    k: "Closure",
                                    v: "Screw Back (5–6.5mm) · Friction Back (5–8mm) · 8mm: Friction Back only",
                                    hl: false,
                                  },
                                ]
                            ...
                            {
                              k: "Certificate",
                              v: "GRA Moissanite Certificate — included with every order",
                              hl: false,
                            },
                            {
                              k: "Health",
                              v: "Hypoallergenic · Nickel-Free · Lead-Free · Cadmium-Free",
                              hl: false,
                            },
                        ...
                        : [
                            ...
                            {
                              k: isRing ? "Setting" : "Clasp",
                              v: isRing ? "Classic 4-Prong Solitaire" : "Double-locking box",
                              hl: false,
                            },
                  ...
                  >
                    <dt className="text-[0.44rem] uppercase tracking-[0.22em] text-muted-foreground/60 shrink-0 pt-0.5 pr-3">
                      {k}
                    </dt>
                    <dd
                      className={`text-[0.72rem] sm:text-[0.76rem] font-semibold text-right min-w-0 break-words ${hl ? "text-foreground" : ""}`}
                    >
                      {v}
                    </dd>
                  </div>
              ...
              {isAnklet ? (
                <div className="space-y-5 text-[0.80rem] leading-[1.90]">
                  ...
                  <p className="text-muted-foreground">
                    ...
                    <strong className="text-foreground font-semibold">
                      D Colorless VVS1 Moissanite
                    </strong>{" "}
                    in a precision round brilliant cut — the highest clarity and color grade
                    available, delivering fire and brilliance that rivals natural diamonds. The{" "}
                    <strong className="text-foreground font-semibold">
                      4-prong claw inlay setting
                    </strong>{" "}
                    maximises stone exposure and light return while securing every stone permanently
                    in place.
                  </p>
                  <p className="text-muted-foreground">
                    Built on a{" "}
                    <strong className="text-foreground font-semibold">
                      solid S925 sterling silver base
                    </strong>{" "}
                    (92.5% pure silver) and finished with{" "}
                    <strong className="text-foreground font-semibold">
                      5 layers of 18K precious metal plating
                    </strong>{" "}
                    — far beyond the 1–2 coats standard in fashion jewellery. Choose from{" "}
                    <strong className="text-foreground font-semibold">18K Yellow Gold</strong> for a
                    warm, timeless lustre,{" "}
                    <strong className="text-foreground font-semibold">18K Rose Gold</strong> for a
                    refined, romantic tone, or{" "}
                    <strong className="text-foreground font-semibold">18K White Gold</strong> for a
                    crisp, contemporary finish.
                  </p>
                  <p className="text-muted-foreground">
                    The <strong className="text-foreground font-semibold">6mm width</strong>{" "}
                    delivers confident, full-presence sparkle at the ankle. A built-in{" "}
                    <strong className="text-foreground font-semibold">
                      2-inch adjustable extender
                    </strong>{" "}
                    fits ankles from 8.5&quot; to 10.5&quot; for a precise, custom fit. The{" "}
                    <strong className="text-foreground font-semibold">
                      double-locking box clasp
                    </strong>{" "}
                    keeps the anklet secured through daily wear, the beach, and every occasion in
                    between.{" "}
                    <strong className="text-foreground font-semibold">
                    ...
                    <strong className="text-foreground font-semibold">
                      GRA moissanite certificate
                    </strong>{" "}
                    verifying the stone's grade, cut, and authenticity. Flat price:{" "}
                    <strong className="text-foreground font-semibold">$249</strong> — regardless of
                    metal finish.
                  </p>
                </div>
              ) : isTennisChain ? (
                <div className="space-y-5 text-[0.80rem] leading-[1.90]">
                  ...
                  <p className="text-muted-foreground">
                    ...
                    in a precision round brilliant cut — the highest clarity and color grade
                    available, delivering fire and brilliance that rivals natural diamonds at a
                  ...
                  <p className="text-muted-foreground">
                    ...
                    keeps the chain secured throughout daily wear, travel, and every occasion in
                    between.{" "}
                    <strong className="text-foreground font-semibold">
                      Hypoallergenic, nickel-free, lead-free, and cadmium-free
                    </strong>{" "}
                ...
                <div className="space-y-5 text-[0.80rem] leading-[1.90]">
                  <p className="text-foreground font-medium">
                    ...
                    <span className="italic">
                      Qureshi Jewelers VVS1 Moissanite Tennis Bracelet.
                    </span>
                  </p>
                  <p className="text-muted-foreground">
                    Every stone is a{" "}
                    ...
                    — the highest clarity and color grade that exists, hand-selected for consistency
                    across the full row. With a refractive index of 2.65–2.69 (exceeding diamond's
                    2.42), moissanite returns more light, more fire, and more brilliance per stone.
                    Each is precision-cut to a{" "}
                    <strong className="text-foreground font-semibold">round brilliant</strong> and
                    locked into a{" "}
                    <strong className="text-foreground font-semibold">
                      4-prong claw inlay setting
                    </strong>{" "}
                  ...
                  <p className="text-muted-foreground">
                    ...
                    (92.5% pure silver — the same standard used in fine jewelry worldwide), finished
                    with{" "}
                    <strong className="text-foreground font-semibold">
                      5 layers of 18K precious metal plating
                    </strong>
                    . Most fashion jewelry ships with 1–2 coats. Ours ships with five. That
                    difference is why our bracelets wear like fine jewelry for years, not months.
                    Choose{" "}
                    <strong className="text-foreground font-semibold">18K Yellow Gold</strong> for a
                    warm, classic look or{" "}
                    <strong className="text-foreground font-semibold">18K White Gold</strong> for a
                    clean, modern finish that pairs with everything.
                  </p>
                  <p className="text-muted-foreground">
                    ...
                    — engineered to stay shut through workouts, travel, and daily wear without a
                    second thought. The entire piece is{" "}
                    <strong className="text-foreground font-semibold">
                      hypoallergenic, nickel-free, lead-free, and cadmium-free
                    </strong>
                    , so it's safe for every skin type, including the most sensitive. Every bracelet
                    ships with a{" "}
                    <strong className="text-foreground font-semibold">
                      GRA Moissanite Certificate
                    </strong>{" "}
                    independently verifying your stone's VVS1 clarity, D color grade, and carat
                    weight — your permanent record of authenticity. Backed by our{" "}
                ...
                product.slug.includes("black") ? (
                  ...
                  <div className="space-y-5 text-[0.80rem] leading-[1.90]">
                    ...
                    <p className="text-muted-foreground">
                      Hypoallergenic, lead-free, nickel-free, and cadmium-free — safe for daily wear
                      on even sensitive skin. Each pair ships with a{" "}
                      <strong className="text-foreground font-semibold">
                  ...
                  <div className="space-y-5 text-[0.80rem] leading-[1.90]">
                    ...
                    <p className="text-muted-foreground">
                      ...
                      and available in{" "}
                      <strong className="text-foreground font-semibold">
                        18K White Gold or 18K Yellow Gold
                      </strong>{" "}
                      plating — five layers deep for lasting durability. The{" "}
                      <strong className="text-foreground font-semibold">
                        threaded screw-back closure
                      </strong>{" "}
                      locks your earrings securely in place — ideal for active wear, travel, or any
                      occasion where you can't afford to lose a stone.
                    </p>
                    <p className="text-muted-foreground">
                      Crafted to be{" "}
                      <strong className="text-foreground font-semibold">
                    ...
                    <p className="text-muted-foreground">
                      ...
                      <strong className="text-foreground font-semibold">3mm</strong> everyday stud
                      to a bold <strong className="text-foreground font-semibold">8mm</strong>{" "}
                      showstopper. Choose the size and metal that matches your style.
                    </p>
                  </div>
                )
              ) : (
              ...
              <div className="grid sm:grid-cols-2 gap-3 mb-5">
                {(isEarring && product.slug.includes("black")
                  ? [
                      {
                        icon: Eye,
                        label: "Genuine Black Moissanite",
                        description:
                          "Not dyed, not coated, not CZ. True black silicon carbide — the same material as moissanite, in a rare opaque black variety.",
                      },
                      {
                        icon: Diamond,
                        label: "Brilliant Cut",
                        description:
                          "Precision 57–58 facets deliver a subtle, sophisticated sparkle — a mirror-like depth that catches light without the flash of colorless stones.",
                      },
                      {
                        icon: ShieldCheck,
                        label: "9.25 Mohs Hardness",
                        description:
                          "Second only to diamond on the Mohs scale. Scratch-resistant and built for daily wear — these studs won't cloud, chip, or fade over time.",
                      },
                      {
                        icon: Award,
                        label: "Certificate of Authenticity",
                        description:
                          "Each pair ships with a moissanite certificate confirming genuine stone composition — not glass, not CZ, not simulant.",
                      },
                    ]
                  : [
            ...
            {isEarring && earringVideo && (
              <div className="border-t border-border pt-8 pb-4">
                <p className="text-[0.50rem] uppercase tracking-[0.30em] text-muted-foreground mb-1">
                  See it in motion
                </p>
      ...
      >
        <div className="flex items-center gap-2 px-4 pt-3 pb-3">
          <div className="min-w-0" style={{ minWidth: 0, maxWidth: "38%" }}>
            <p className="text-[0.38rem] uppercase tracking-[0.18em] text-muted-foreground mb-0.5 truncate">
              {mobileConfigLabel}
            </p>
            <p className="font-display text-[1.1rem] leading-none truncate">
              {formatUSD(price * qty)}
            </p>
          </div>
          <button
          ...
          <section className="bg-cream border-y border-border">
            <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-10 py-12 sm:py-16 lg:py-20">
              ...
              <div className="mt-8 text-center sm:hidden">
                <Link
                  to="/shop"
                  className="text-[0.55rem] uppercase tracking-[0.28em] text-muted-foreground hover:text-foreground transition-colors"
                >
